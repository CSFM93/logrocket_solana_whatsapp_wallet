...
def create_account(phone_number):
    try:
        kp = Keypair.generate()
...
def fund_account(phone_number, amount):
    try:
        amount = int(1000000000 * amount)
        account = load_wallet(phone_number)
...
def send_sol(phone_number, amount, receiver):
    try:
        account = load_wallet(phone_number)
